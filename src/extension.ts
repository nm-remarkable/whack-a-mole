...
import { NukeDiagnostic, ConstDiagnostic, SwearDiagnostic, LoggingDiagnostic, FakeDiagnostic } from './fakeDiagnostics';

const fakeDiagnostic: Map<string, FakeDiagnostic> = new Map();
const diagnosticTypes = [NukeDiagnostic, ConstDiagnostic, SwearDiagnostic, LoggingDiagnostic];
const maxFakeDiagnostics = 2;

export function activate(context: vscode.ExtensionContext) {

    // Create a collection to report diagnostics to VS Code
    const diagnosticCollection = vscode.languages.createDiagnosticCollection('battleship');
    ...
    setInterval(() => {
        ...
        fakeDiagnostic.forEach((fake) => vscodeDiagnostics.push(fake.transform()));
...
function createFakeDiagnostic(document: vscode.TextDocument) {
    const diagnosticType = diagnosticTypes[Math.floor(Math.random() * diagnosticTypes.length)];
